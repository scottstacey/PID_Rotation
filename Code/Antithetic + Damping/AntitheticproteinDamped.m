function AntitheticproteinDamped
clear; clc; clf;
% This model needs to be updated to reflect changes in the non-damped
% equivalent model!
...
figure(1);
set(gca, 'fontsize', 14);
plot(t, S(:,1), 'g--', t, S(:,2), 'r--', t, S(:,3), 'g', t, S(:,4), 'r',t, S(:,5), 'b--', t, S(:,6), 'b', t, S(:,7), 'y', 'LineWidth', 3);
legend('Z1 mRNA', 'Z2 mRNA', 'Z1 Protein', 'Z2 Protein', 'X mRNA', 'X Protein', 'X*', 'Location', 'northeast');
xlabel('Time (Hours)');
...
dX_pdt  = (T_2 * X_m) - (delta_p * X_p) - (alpha_1 * X_p) + (alpha_2 * X_star);
dX_stardt = (alpha_1 * X_p) - (alpha_2 * X_star) - (delta_star * X_star);


dS = [dZ_1mdt; dZ_2mdt; dZ_1pdt; dZ_2pdt; dX_mdt; dX_pdt; dX_stardt];
