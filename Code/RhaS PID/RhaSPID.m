...
global deg       % Degradation rate due to a degradation tag
...
k_4      = 0;              % Set to zero for PI controller, otherwise non-zero
alpha_3  = 1 * 60 * 60;
alpha_4  = 1 * 60 * 60;
deg      = 0;             % Degradation rate due to a degradation tag
delta_3  = delta_1 + deg; % Dilution Rate + Degradation Rate 
delta_4  = delta_1 + deg; % Dilution Rate + Degrdation Rate 
...
dRhaSdt      = (k_2 * Z_1) - (delta_1 * RhaS) - (2 * alpha_1 * RhaS * RhaS) + (2 * alpha_2 * X) - (alpha_3 * RhaS * RhaS_star) + (alpha_4 * X_star) + (deg * X_star);
